...
Prerequisites:
  ...
  - The canonical `eIntersects(geometry, th3index)` overload
...
Prefilter pattern (single recipe across geometry types):
  ...
  `eIntersects(geometry, th3index)` accepts any GEOMETRY (POINT/LINESTRING/
  POLYGON/MULTI*/GeometryCollection), walking it to an H3 cell set at the
  th3index resolution and returning TRUE iff the trip's th3index path ever
  lies in any of those cells.  The prefilter is
...
WHERE eIntersects(R.Geom, T.trip_h3)
...
WHERE T1.VehId < T2.VehId
  AND eIntersects(R.Geom, T1.trip_h3)
  AND eIntersects(R.Geom, T2.trip_h3)
...
WHERE eIntersects(P.Geom, T.trip_h3)
